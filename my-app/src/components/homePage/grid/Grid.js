...
class Grid extends Component {
    ...
    render() {
        ...
        var allRows = []; // Array which will contain the rows 

        var len = this.props.items.length; // Length of the items array passed to the component

        // Number of rows = Number of items / Desired number of items in each row
        var totalRows = Math.ceil(len / this.props.rowLength);
        ...
        while(countRows < totalRows){
            ...
            let newRow = []; // Represents the current row being created
